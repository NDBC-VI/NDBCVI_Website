            ...
            {children}
            {
                ...
                createPortal(
                    <div id='backdrop' onClick={handleClose} onScroll={handleScroll} className={`flex flex-col items-center z-20 transition-colors overflow-y-scroll m-0 ${isOpen ? "fixed inset-0 bg-black/70" : ""}`}>
                        <div id="infoPopup" onClick={(e) => e.stopPropagation()} className={`h-full w-11/12 overflow-y-scroll mt-[80px] mb-[60px] mx-[12px] pt-[24px] pb-[36px] md:overflow-visible md:h-fit md:mt-[172px] md:mb-[63px] md:w-11/12 md:pt-[72px] md:pb-[10px] rounded-3xl bg-white transition-all duration-500 ${isOpen ? "scale-100 opacity-100" : 'scale-125 opacity-0'}`}>
