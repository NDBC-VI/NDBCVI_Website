...
export const EventScrollLinks = ({events, displayedEvent, scrollFn}: {events: EventScrollLinkType[], displayedEvent: number, scrollFn: (e: React.MouseEvent, index: number) => void}) => {
    ...
    useEffect(() => {
        ...
        const eventsVisibilityPoint = window.innerWidth <= 768 ? eventsSection.offsetTop : eventsSection.offsetTop + eventsSection.offsetHeight;
        const calendarVisibilityPoint = window.innerWidth <= 768 ? calendarSection.offsetTop : calendarSection.offsetTop + calendarSection.offsetHeight;
