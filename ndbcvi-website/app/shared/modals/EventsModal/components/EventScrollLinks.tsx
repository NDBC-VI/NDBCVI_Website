            ...
            {   domReady && isOpen && createPortal(     
                    <div className='absolute bottom-0 left-0 w-full flex justify-center cursor-pointer'>
                        <div className='w-11/12 scale-x-[1.005] flex flex-col md:hidden'>
                            <div className={`w-full z-10 bg-[#1D1841] rounded-b-3xl text-white`} onClick={toggleDropdown}>
                            ...
                            <div className={`w-full h-[60px] bg-[#1D1841] text-white z-[9] pl-[20px] pt-0 pb-[18px] rounded-b-3xl flex items-end overflow-x-scroll relative bottom-[50px] opacity-0 transition-all linear duration-300 ${isDropdownOpen ? "translate-y-[30px] opacity-100 z-12" : ""}`}>
                                {
                                    events?.map((section: EventScrollLinkType, index: number) => (
